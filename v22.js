(function(){
  ...
  window.openPunch=function(){pFraction=1;inject();oldOpenPunch();setTimeout(markPunch,0)};
  ...
  window.openEdit=function(name,date){inject();oldOpenEdit(name,date);const r=rec(data.workshops[active],name,date);eFraction=Number(r&&r.workFraction||1)===.5?.5:1;setTimeout(markEdit,0)};
